...
module.exports = {
    ...
    findHistory(value, tb_users_u_id) {
        return new Promise((resolve, reject) => {
            ...
            const sqls = {
                count: `SELECT COUNT(*) AS row_count FROM ${table.a} JOIN tb_detailjob ON tb_apply.tb_detailjob_j_id = tb_detailjob.j_id WHERE tb_users_u_id = ${connection.escape(tb_users_u_id)}`,
                select: `SELECT * FROM ${table.a} JOIN tb_detailjob ON tb_apply.tb_detailjob_j_id = tb_detailjob.j_id WHERE tb_users_u_id = ${connection.escape(tb_users_u_id)}`
            };
    ...
    findHistorys(value, tb_owner_o_id) {
        return new Promise((resolve, reject) => {
            const limitPage = config.limitPage;
            const startPage = ((value.page || 1) - 1) * limitPage;
            const sqls = {

...
module.exports = {
    ...
    findDetailJob(id) {
        return new Promise((resolve, reject) => {
            ...
            SELECT * 
            FROM tb_detailjob 
            JOIN tb_apply ON tb_apply.tb_detailjob_j_id = tb_detailjob.j_id
            WHERE a_id = ?`, [id], (error, result) => {
                    if (error) return reject(error);
                    resolve(result.length == 0 ? null : result[0]);
                });
        });
    ...
    findHistory(value) {
        return new Promise((resolve, reject) => {
            ...
            const startPage = ((value.page || 1) - 1) * limitPage;
            const sqls = {
                count: `SELECT COUNT(*) AS row_count FROM ${table} JOIN tb_owner ON tb_detailjob.tb_owner_o_id = tb_owner.o_id WHERE tb_owner_o_id `,
                select: `SELECT * FROM ${table} JOIN tb_owner ON tb_detailjob.tb_owner_o_id = tb_owner.o_id WHERE tb_owner_o_id `
            };

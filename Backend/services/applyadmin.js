...
module.exports = {
    ...
    findHistory(value) {
        return new Promise((resolve, reject) => {
            ...
            const sqls = {
                count: `SELECT COUNT(*) AS row_count FROM ${table.a} JOIN tb_detailjob ON tb_apply.tb_detailjob_j_id = tb_detailjob.j_id WHERE tb_users_u_id = ?`,
                select: `SELECT * FROM ${table.a} JOIN tb_detailjob ON tb_apply.tb_detailjob_j_id = tb_detailjob.j_id WHERE tb_users_u_id = ? `
            };
    ...
    findHistorys(value) {
        return new Promise((resolve, reject) => {
            ...
            const startPage = ((value.page || 1) - 1) * limitPage;
            const sqls = {
                count: `SELECT COUNT(*) AS row_count FROM ${table.a} JOIN tb_detailjob ON tb_apply.tb_detailjob_j_id = tb_detailjob.j_id JOIN tb_owner ON tb_detailjob.tb_owner_o_id = tb_owner.o_id WHERE tb_owner_o_id `, 
                select: `SELECT * FROM tb_apply JOIN tb_detailjob ON tb_apply.tb_detailjob_j_id = tb_detailjob.j_id JOIN tb_owner ON tb_detailjob.tb_owner_o_id = tb_owner.o_id WHERE tb_owner_o_id `
    };
            if (value.search_key && value.search_text) {
                const key = value.search_key;
                const txt = value.search_text;

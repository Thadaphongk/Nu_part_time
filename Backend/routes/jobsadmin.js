...
const service = require('../services/jobsadmin');
const jobService = require('../services/jobsadmin');
...
router.get('/historys', [
    ...
    try {
        req.validate();
        res.json(await jobService.findHistory(req.query));
    }
    catch (ex) { res.error(ex); }
});
...
router.post('/', [
    ...
    check('j_day').not().isEmpty(),
    check('j_hour').not().isEmpty(),
    check('j_salary').not().isEmpty(),

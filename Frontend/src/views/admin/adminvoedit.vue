<template>
    <Layout>
        <div class="card">
            <div class="card-body">
                <header>
                    <h5><i class="fa fa-edit"></i> แก้ไข ข้อมูลงาน</h5>
                </header>
                <hr>
      ...
      <p class="invalid-feedback">{{ errors.first('j_phone') }}</p>
                    ...
                    <div class="form-group mt-4 mb-5">
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <router-link :to="{ name: 'adminvo' }" class="btn btn-danger btn-block">
                                    ยกเลิก
                                </router-link>
                            </div>
                        </div>
  ...
  components: {
    ...
    created() {
      ...
      Axios.get('/api/ownerset/getownerlogin') // Replace with your actual API endpoint
        .then(response => {
  ...
  methods: {
    ...
    onSubmit() {
      this.$validator.validateAll().then(valid => {
        ...
        const request = isNaN(updateId)
          ? Axios.post("/api/jobs", this.form) // สร้างใหม่
          : Axios.put(`/api/jobs/${updateId}`, this.form); // แก้ไข
        // ส่งข้อมูลไปหา Server
        request
          .then(res => this.alertify.success("ทำรายการสำเร็จ"),this.$router.push({ name: "adminvo" }))
    ...
    initialUpdateItem() {
      ...
      Axios.get(`/api/jobs/${id}`)
        ...
        .catch(() => this.$router.push({ name: "adminvo" }));
    }
